...
interface PlantData {
    ...
    boxIndex?: number //底部盒子的索引（0, 1, 2...），仅用于底部植物，-1表示不是底部植物
}
...
export default class GameUI extends cc.Component {
    ...
    private initBottomPlantBox() {
        ...
        let plantConfigs = [
            ...
            { type: plantType.cao, level: 4, x: 45, y: 30, targetMapPlantIndex: 0, isClickable: false, boxIndex: 2 } // 第三个（索引2）
        ]

        plantConfigs.forEach((config, index) => {
            this.createPlantInBox(config.type, config.level, config.x, config.y, config.targetMapPlantIndex, config.isClickable, config.boxIndex)
        })
    }
     ...
     * 在底部植物盒子中创建植物
     */
    private createPlantInBox(type: number, level: number, x: number, y: number, targetMapPlantIndex: number = -1, isClickable: boolean = true, boxIndex: number = -1): PlantData | null {
        if (!this.plantPre) {
